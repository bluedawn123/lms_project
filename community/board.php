...
// 총 게시물 수 계산
$total_items_sql = "SELECT COUNT(*) AS total FROM board WHERE category = 'free'";
...
$sql = "SELECT pid, title, content, user_id, hit, likes, date FROM board WHERE category = 'free' ORDER BY date DESC LIMIT ?, ?";
...
<div class="community container">
  ...
  <?php foreach ($qnas as $index => $qna): ?>
    <tr>
      ...
      <td id="hit-<?= $qna['pid'] ?>"><?= htmlspecialchars($qna['hit']) ?></td>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="inquiryModalLabel">글쓰기</h5>
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.status === "success") {
          ...
          const hitElement = document.getElementById(`hit-${postId}`);
          if (hitElement) {
            hitElement.textContent = parseInt(hitElement.textContent) + 1;
          }
        } else {
...
function fetchComments(postId) {
  $.ajax({
    url: "/qc/community/get_comments.php",
    type: "GET",
    data: { post_id: postId },
    success: function (response) {
      if (response.status === "success") {
...
$("#inquiryButton").on("click", function () {
  const freeModal = new bootstrap.Modal(document.getElementById("inquiryModal"));
  freeModal.show();
});
